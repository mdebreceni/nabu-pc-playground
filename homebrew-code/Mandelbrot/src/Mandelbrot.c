// Mandelbrot.c - Render the Mandelbrot Set with a NABU PC in 64 x 48 multicolor
// mode Copyright Mike Debreceni 2022
//
//
...
uint8_t cursor_sprite_small[] = {0xff, 0x81, 0x81, 0x81,
                                 0x81, 0xff, 0x00, 0x00};
...
int iterationCount(float cr, float ci, bool (*callback_func)(void)) {
    ...
    // return -1 if callback_func returns false (indicating we should interrupt
    // calculation)
    int i = 0;
    float zr = 0, zi = 0;
    float temp;
